...
public class AWTImage implements BaseImage {
    ...
    public static class AWTDraw implements Draw {
        ...
        public void floodFill(int x, int y, Color c, Color edge, boolean keepAlpha) {
            //orig(x, y, c, edge, keepAlpha);
            if(edge == null) {
                noEdgeFill(x, y, c, keepAlpha);
            } else {
                edgeFill(x, y, c, edge, keepAlpha);
            }
        }
        public void edgeFill(int x, int y, Color c, Color edge, boolean keepAlpha) {
        ...
        public void noEdgeFill(int x, int y, Color c, boolean keepAlpha) {
            if(x < 0 || x >= ai.getWidth()) {
                return;
            }
            if(y <0 || y>=ai.getWidth()) {
